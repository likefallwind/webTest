...
from writePoem.data import get_data
from writePoem.model import PoetryModel
from writePoem.utils import Visualizer
from IPython import embed
import datetime


class Config(object):
    data_path = 'writePoem/data/'  # 诗歌的文本文件存放路径
    pickle_path = 'writePoem/tang.npz'  # 预处理好的二进制文件
    author = None  # 只学习某位作者的诗歌
    constrain = None  # 长度限制
    category = 'writePoem/poet.tang'  # 类别，唐诗还是宋诗歌(poet.song)
    lr = 1e-3
    weight_decay = 1e-4
    use_gpu = False
    epoch = 20
    ...
    debug_file = 'writePoem/debug/debug.txt'
    model_path = 'writePoem/checkpoints/tang_199.pth'  # 预训练模型路径
    prefix_words = '窗前明月光，疑似地上霜。'  # 不是诗歌的组成部分，用来控制生成诗歌的意境
    start_words = '谢华'  # 诗歌开始
    acrostic = True  # 是否是藏头诗
    model_prefix = 'writePoem/checkpoints/tang'  # 模型保存路径


#opt = Config()


def generate(opt, model, start_words, ix2word, word2ix, prefix_words=None):
    """
...
def userTest( acrostic = True, start_words = "谢华"):
    opt = Config()
    data, word2ix, ix2word = get_data(opt)
    opt.start_words = start_words
    model = PoetryModel(len(word2ix), 128, 256);
    map_location = lambda s, l: s
    state_dict = t.load(opt.model_path, map_location=map_location)
    model.load_state_dict(state_dict)
    ...
    start_words = start_words.replace(',', u'，') \
        ...
        .replace('?', u'？')
    if acrostic:
        result = gen_acrostic(opt, model, start_words, ix2word, word2ix, prefix_words)
    else:
        result =  generate(opt, model, start_words, ix2word, word2ix, prefix_words)
    return (''.join(result))
